...
//     auto h_electrons_p =  m_histContainer->get1DHist("electrons_p", 125, 0., 250.);
    ...
    // non-corrected momentum
//     varMember<ROOT::VecOps::RVec<float>> jet_px_nc_ {tree, "jet_px_corr"}; // for the moment don't have non-corrected saved, so just going to use corrected vals here.
//     varMember<ROOT::VecOps::RVec<float>> jet_py_nc_ {tree, "jet_py_corr"}; // for the moment don't have non-corrected saved, so just going to use corrected vals here.
//     varMember<ROOT::VecOps::RVec<float>> jet_pz_nc_ {tree, "jet_pz_corr"}; // for the moment don't have non-corrected saved, so just going to use corrected vals here.
//     varMember<ROOT::VecOps::RVec<float>> jet_e_nc_ {tree, "jet_e_corr"}; // for the moment don't have non-corrected saved, so just going to use corrected vals here.
  
//     varMember<ROOT::VecOps::RVec<float>> jet_px_nc {tree, "jet_px_corr"}; // for the moment don't have non-corrected saved, so just going to use corrected vals here.
//     varMember<ROOT::VecOps::RVec<float>> jet_py_nc {tree, "jet_py_corr"}; // for the moment don't have non-corrected saved, so just going to use corrected vals here.
//     varMember<ROOT::VecOps::RVec<float>> jet_pz_nc {tree, "jet_pz_corr"}; // for the moment don't have non-corrected saved, so just going to use corrected vals here.
//     varMember<ROOT::VecOps::RVec<float>> jet_e_nc {tree, "jet_e_corr"}; // for the moment don't have non-corrected saved, so just going to use corrected vals here.
  
    // corrected momentum
    varMember<ROOT::VecOps::RVec<float>> jet_px {tree, "jet_px_corr"}; // for the moment don't have non-corrected saved, so just going to use corrected vals here.
    varMember<ROOT::VecOps::RVec<float>> jet_py {tree, "jet_py_corr"}; // for the moment don't have non-corrected saved, so just going to use corrected vals here.
    varMember<ROOT::VecOps::RVec<float>> jet_pz {tree, "jet_pz_corr"}; // for the moment don't have non-corrected saved, so just going to use corrected vals here.
    varMember<ROOT::VecOps::RVec<float>> jet_e {tree, "jet_e_corr"}; // for the moment don't have non-corrected saved, so just going to use corrected vals here.
...
//             std::cout << "jet_px.at("<<lv<<"): " << jet_px.at(lv) << std::endl;
//             std::cout << "jet_py.at("<<lv<<"): " << jet_py.at(lv) << std::endl;
//             std::cout << "jet_pz.at("<<lv<<"): " << jet_pz.at(lv) << std::endl;
//             std::cout << "jet_e.at("<<lv<<"): " << jet_e.at(lv) << std::endl;
         
//             std::cout << "jet_px_nc.at("<<lv<<"): " << jet_px_nc.at(lv) << std::endl;
//             std::cout << "jet_py_nc.at("<<lv<<"): " << jet_py_nc.at(lv) << std::endl;
//             std::cout << "jet_pz_nc.at("<<lv<<"): " << jet_pz_nc.at(lv) << std::endl;
//             std::cout << "jet_e_nc.at("<<lv<<"): " << jet_e_nc.at(lv) << std::endl;
         
            TLorentzVector LVjet(jet_px.at(lv), jet_py.at(lv), jet_pz.at(lv), jet_e.at(lv));
            LVjets.push_back(LVjet);
            //TLorentzVector LVjet_nc(jet_px_nc.at(lv), jet_py_nc.at(lv), jet_pz_nc.at(lv), jet_e_nc.at(lv));
            TLorentzVector LVjet_nc(jet_px.at(lv), jet_py.at(lv), jet_pz.at(lv), jet_e.at(lv));
            LVjets_nc.push_back(LVjet_nc);
        }
      
       // Flav scores of each jet
       std::array<float,7> j0_flav {recojet_isB.at(0), recojet_isC.at(0), recojet_isS.at(0), recojet_isD.at(0), recojet_isU.at(0), recojet_isG.at(0), recojet_isTAU.at(0)};
       std::array<float,7> j1_flav {recojet_isB.at(1), recojet_isC.at(1), recojet_isS.at(1), recojet_isD.at(1), recojet_isU.at(1), recojet_isG.at(1), recojet_isTAU.at(1)};
       std::array<float,7> j2_flav {recojet_isB.at(2), recojet_isC.at(2), recojet_isS.at(2), recojet_isD.at(2), recojet_isU.at(2), recojet_isG.at(2), recojet_isTAU.at(2)};
       std::array<float,7> j3_flav {recojet_isB.at(3), recojet_isC.at(3), recojet_isS.at(3), recojet_isD.at(3), recojet_isU.at(3), recojet_isG.at(3), recojet_isTAU.at(3)};
