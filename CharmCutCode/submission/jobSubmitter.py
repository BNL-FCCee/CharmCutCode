...
class submission:
    def __init__(self, args):
        ...
        self.submissionFolder = "submission"
        self.jobListFolder = "jobList"

        os.system("mkdir -vp " + self.submissionFolder)
    ...
    def _remergeReplitSubjobs(self, size):
        ...
        os.system("mkdir -p %s/%s/%s" % (self.jobListFolder, basePath, "mergeJob"))
        import random
        ...
        for job in flattenJobList:
            jobName = "source %s/%s/%s.sh" %(self.jobListFolder, job["jobOutPath"],job["jobOutName"])
        ...
        counter = 0
        filePath = "%s/%s/%s/" % (self.jobListFolder, basePath, "mergeJob")
        mergedSubjobList = []
...
class doDryRun(submission):
    ...
    def submitJob(self):
        ...
        for subJob in self.flatJobList:
            jobList = self.jobListFolder + "/" + subJob["jobOutPath"] + "/" + subJob["jobOutName"] + ".sh"
            print (jobList)
...
class submitCondor(submission):
    ...
    def _makeCondorExecutable(self, subDir):
        executableName = self.submissionFolder + "/" + subDir + "/condorExe.sh"
    ...
    def _makeCondorSteer(self, currJob, subDir):
        executableName = self.submissionFolder + "/" + subDir + "/condorSteer.sub"

        fileObj = open(executableName, 'w')
        fileObj.write("Executable = %s/%s/condorExe.sh \n" % (self.submissionFolder, subDir))
        fileObj.write("Universe = vanilla \n")
        fileObj.write("Notification = Error \n")
        fileObj.write("GetEnv = True \n\n")

        for subJob in currJob:
            condorPath = self.submissionFolder + "/" + subJob["jobOutPath"]
            # if the folder doesn't exist create it
            if(not os.path.isdir(condorPath)):
                os.system("mkdir -p %s" % (condorPath))
            
            fileObj.write("Arguments = %s/%s/%s.sh  \n" %(self.jobListFolder, subJob["jobOutPath"],subJob["jobOutName"]))
            fileObj.write("Output    = %s/%s/%s.out \n" %(self.submissionFolder, subJob["jobOutPath"],subJob["jobOutName"]))
            fileObj.write("Error     = %s/%s/%s.err \n" %(self.submissionFolder, subJob["jobOutPath"],subJob["jobOutName"]))
            fileObj.write("Log       = %s/%s/%s.log \n" %(self.submissionFolder, subJob["jobOutPath"],subJob["jobOutName"]))
            fileObj.write("Queue \n\n")
    ...
    def process(self):
        ...
        directory = self.submissionFolder
        ...
        print("Condor submission scripts written")

    def submitJob(self):
        if(self.args.dryRun):
            print ("Dry run of your condor jobs. Here are the submit commands")
...
class runLocal(submission):
    ...
    def submitJob(self):
        if(self.args.dryRun):
            print ("Dry run of your local jobs.")
            for job in self.flatJobList:
                jobListPath = self.jobListFolder + "/" + job["jobOutPath"] +"/" + job["jobOutName"] +  ".sh"
                print(jobListPath)
            return
...
class runInteractive(submission):
    ...
    def submitJob(self):
        if(self.args.dryRun):
            print ("Dry run of your interactive job/")
            for job in self.flatJobList:
                jobListPath = self.jobListFolder + "/" + job["jobOutPath"] +"/" + job["jobOutName"] +  ".sh"
                print(jobListPath)
            return
...
class submitCedarContainer(submission):
    ...
    def _makeCedarContainerExecutable(self, subJob, subDir):
        fileName = self.submissionFolder + "/" + subDir + "/" + subJob["jobOutName"] + ".sh"
        fileObj = open(fileName, 'w')
        outFileName = "stdout_" + subJob["jobOutName"] + ".txt"
        CWD = os.getcwd()
        jobList = CWD + "/"+self.jobListFolder+"/" + subDir + "/" + subJob["jobOutName"] + ".sh"
    ...
    def submitJob(self):
        if(self.args.dryRun):
            ...
            for cmd in self.submitCommands:
                os.system(cmd)
                #print(cmd)
            print("Slurm container submission completed")
